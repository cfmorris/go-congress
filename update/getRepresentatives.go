...
func GetRepresentatives() {
	config, _ := config.ReadConfig("./config.yaml")
	fmt.Println("updating Representatives....")
	for i := 102; i <= 117; i++ {
		time.Sleep(3 * time.Second)
		house := "House" // "Senate" or "House"
		...
		for _, k := range people1.Results {
			for b, l := range k.Members {
				...
				session := driver.NewSession(neo4j.SessionConfig{DatabaseName: config.Database.Username})
				defer session.Close()
				...
				_, err = session.WriteTransaction(
					func(tx neo4j.Transaction) (interface{}, error) {
						...
						MERGE (house:Congress:House {number: $congressNumber })
						MERGE (member:Person:CongressMembers:Representative { id: $id })
						SET
